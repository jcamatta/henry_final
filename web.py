...
def get_classification(review):
    """ Utiliza el modelo de yelp para clasificar entre useful, funny, y cool. """
    response = requests.get(f'http://127.0.0.1:8000/business/topic_classification/{review}').content
...
def get_reviews(dataframe, use_google):
    ...
    for k in range(len(companies)):
        ...
        temp_df['name'] = name
        companies_data = pd.concat([companies_data, temp_df])
    if 'reviews' in companies_data.columns:
        companies_data['topic'] = companies_data['reviews'].apply(get_classification).str.title()
...
with left:
   # Mapa
    m = folium.Map(location=CENTER_START, zoom_start=5, min_zoom=3, max_zoom=14, max_bounds=True,
...
if 'data' in st.session_state:
    st.data_editor(get_reviews(st.session_state['data'], use_google), hide_index=True, use_container_width=True, 
                   column_order=['name', 'reviews', 'stars', 'topic'],
                   column_config={
                       'name': st.column_config.TextColumn('Business Name', disabled=True),
                       'reviews': st.column_config.TextColumn('Reviews', disabled=True),
                       'stars': st.column_config.NumberColumn(label='Average of Stars', disabled=True, format='%d ⭐'),
                       'topic': st.column_config.TextColumn(label='Topic Classification', disabled=True,)
                   })
